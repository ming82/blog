  ...
  <div class="shadow" align="left">
    ...
    <div>
      ...
      <a style="float: right;font-size: 12px;cursor: pointer;color: dodgerblue;margin-right: 20px"
         @click="answer(comment.commentId,comment.userNickname)">回复</a>
    </div>
    <br>
    <div class="comment" style="margin-left: 40px;background-color: #f2f2f2" v-if="isShowAnswer">
      <div v-for="(commentAnswer,index) in comment.commentAnswers" :key="index">
        <img @click="toUser(commentAnswer.userId)" :src="commentAnswer.userPicture" class="image"
             style="cursor:pointer;width: 30px;height: 30px;border-radius: 50px">
        <a @click="toUser(commentAnswer.userId)" style="cursor: pointer"
           class="namehref">{{commentAnswer.userNickname}}</a>
        &nbsp;&nbsp;&nbsp;&nbsp;回复&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <a style="float: right;font-size: 12px;cursor: pointer;color: #909399;margin-right: 20px"
           @click="answer(comment.commentId,commentAnswer.userNickname)">回复</a>
